...
## Testing Guidelines
Vitest is configured for Node and discovers `src/**/*.test.ts`. Follow the current pattern in [`src/__tests__/smoke.test.ts`](/home/godja/Dev/ai-gateway/src/__tests__/smoke.test.ts): cover HTTP behavior with `supertest`, mock providers before importing the app, and set required env vars inside the test process. Add tests for auth, validation, provider routing, and response-shape changes.
...
Never commit real API keys or `GATEWAY_MASTER_KEY` values. Keep secrets in local env files or deployment config. When editing auth, CORS, rate limiting, or readiness behavior in [`src/index.ts`](/home/godja/Dev/ai-gateway/src/index.ts), preserve the probe split: `/health` and `/ping` stay cheap public liveness endpoints, while `/ready` remains the public readiness check used for deployment and container health.
