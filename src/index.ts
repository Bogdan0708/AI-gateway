...
import { validateChatCompletion, validateSimpleCompletion } from './middleware/validate';
...
app.get('/health', (_req: Request, res: Response) => {
  const enabledProviders = listProviders().filter((provider) => provider.enabled).map((provider) => provider.id);
...
app.post('/v1/chat/completions', validateChatCompletion, async (req: Request, res: Response) => {
  try {
    const { messages, provider, model, max_tokens, temperature, tenant_id, task_type } = req.body as {
...
app.post('/complete', validateSimpleCompletion, async (req: Request, res: Response) => {
  try {
    const { prompt, system, provider, model, max_tokens, temperature } = req.body as {
      prompt: string;
      system?: string;
      provider?: string;
      model?: string;
